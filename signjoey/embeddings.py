...
from signjoey.transformer_layers import PositionalEncoding, PositionwiseFeedForward


def get_activation(activation_type, inplace=False):
...
class SpatialEmbeddings(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.tin_linear1 = nn.Sequential(
            nn.Conv1d(self.input_size, 512, kernel_size=5, stride=1, padding=0),   # 768 512
            nn.BatchNorm1d(512),
            get_activation('relu', inplace=True),
            nn.MaxPool1d(kernel_size=1, ceil_mode=False)
        )
        self.tin_linear2 = nn.Sequential(
            nn.Conv1d(512, embedding_dim, kernel_size=5, stride=1, padding=0),
    ...
    # pylint: disable=arguments-differ
    def forward(self, x: Tensor, mask: Tensor, src_len: Tensor = None) -> Tensor:
        """
        :param mask: frame masks
        :param x: input frame features
        :param src_len: input frame features ori len
        :return: embedded representation for `x`
        """
