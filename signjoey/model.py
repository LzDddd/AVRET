...
from signjoey.transformer_layers import AdaptiveFusion, AdaptiveMask
...
class SignModel(nn.Module):
    ...
    def __init__(
        ...
        self.af = AdaptiveFusion(input_size_1=self.encoder.hidden_size, input_size_2=self.encoder.hidden_size)
        self.first_am = AdaptiveMask(
            input_size=self.sgn_embed.input_size, output_size=self.encoder.hidden_size, dropout=0.1)
        self.snd_am = AdaptiveMask(
            input_size=self.encoder.hidden_size, output_size=self.encoder.hidden_size, dropout=0.1)
    ...
    def forward(
        ...
        _, lstm_1, am_mask1 = self.first_am(sgn, sgn_lengths, k=2, mask=sgn_mask)

        encoder_output, encoder_hidden = self.encode(
            sgn=sgn, sgn_mask=am_mask1, sgn_length=sgn_lengths
        )

        # AF module
        encoder_output = self.af(encoder_output, lstm_1)

        # Second AM module
        _, lstm_o, am_mask2 = self.snd_am(encoder_output, sgn_lengths, k=4, mask=am_mask1)
    ...
    def run_batch(
        ...
        """
        _, lstm_1 = self.first_am(batch.sgn, batch.sgn_lengths)
