...
class GateFusionMechanism(nn.Module):
    """
    ...
    def __init__(self, input_size_1=512, input_size_2=512, output_siz=2, bias=False):
        ...
        :param bias:
        """
        super(GateFusionMechanism, self).__init__()
        ...
        # self.layer_norm = nn.LayerNorm(input_size_1, eps=1e-6)
...
class DDEM(nn.Module):
    ...
    def __init__(self, input_size=512, output_size=512, dropout=0.1):
        ...
        :param output_size: dimensionality of output representation
        :param dropout:
        """
        super(DDEM, self).__init__()
        self.lstm = BiLSTMLayer(input_size=input_size, hidden_size=output_size, dropout=dropout)
        self.fc = nn.Sequential(
            nn.Linear(output_size, 475),  # 475/263
            nn.Softmax(dim=-1)
        )
        self.layer_norm = nn.LayerNorm(output_size, eps=1e-6)  # eps=1e-6

    def forward(self, input_tensor, input_len, k=20):
        lstm_o = self.lstm(input_tensor, input_len, input_tensor.shape[1])
        list_out = self.fc(lstm_o[:, -1, :])
...
class DDEM2(nn.Module):
    ...
    def forward(self, input_tensor, input_len, k=20):
        lstm_o = self.lstm(input_tensor, input_len, input_tensor.shape[1])
        list_out = self.softmax(lstm_o[:, :, -1])
        values, indices = list_out.topk(k, dim=-1, largest=False, sorted=False)
        lstm_o = self.layer_norm(lstm_o)
...
class PositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, input_size, ff_size, dropout=0.1, use_gfm=False):
        ...
        if use_gfm:
            self.ff_gfm = GateFusionMechanism()

    def forward(self, x):
        x_norm = self.layer_norm(x)
        if self.use_gfm:
            return self.ff_gfm(self.pwff_layer(x_norm), x)
        else:
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        # use GateFusionMechanism
        # self.encoder_mha_gfm = GateFusionMechanism()
        # FSDT layers
        self.clips_layer_norm = nn.ModuleList([nn.LayerNorm(size, eps=1e-6) for _ in range(32)])
        self.clips_mha_blocks = nn.ModuleList([MultiHeadedAttention(num_heads, size, dropout) for _ in range(32)])
        ...
        self.clips_dropout = nn.ModuleList([nn.Dropout(dropout) for _ in range(32)])
    ...
    def forward(self, x: Tensor, mask: Tensor, src_length=None) -> Tensor:
        ...
        h = self.dropout(h) + x
        # h = self.encoder_mha_gfm(self.dropout(h), x)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        # use af
        # self.decoder_masked_mha_gfm = GateFusionMechanism()
        # self.decoder_mha_gfm = GateFusionMechanism()
    ...
    def forward(
        ...
        # h1 = self.decoder_masked_mha_gfm(self.dropout(h1), x)
        ...
        # o = self.feed_forward(self.decoder_mha_gfm(self.dropout(h2), h1))
