...
from signjoey.loss import XentLoss, KLLoss
...
def validate_on_data(
    ...
    visual_loss_function: torch.nn.Module = None,
    txt_loss_function: torch.nn.Module = None
) -> (
    float,
    float,
    float,
    ...
    :param visual_loss_function: frame subsampling ratio
    :param txt_loss_function: frame subsampling ratio
    :return:
    ...
    with torch.no_grad():
        ...
        for valid_batch in iter(valid_iter):
            batch = Batch(
                ...
                random_text_masking_ratio=True
            )
            sort_reverse_index = batch.sort_by_sgn_lengths()

            total_loss = model.get_loss_for_vlp_pretrain_batch(
                batch=batch,
                ...
                visual_loss_function=visual_loss_function,
                txt_loss_function=txt_loss_function
            )
            if do_recognition:
                total_recognition_loss += batch_recognition_loss
                total_num_gls_tokens += batch.num_gls_tokens
            if do_translation:
                total_translation_loss += total_loss
...
def test(
    ...
    visual_loss_function = KLLoss().cuda() if use_cuda else KLLoss()
    txt_loss_function = KLLoss().cuda() if use_cuda else KLLoss()
    ...
    # Prediction
    if do_recognition:
        # Dev and Test Recognition CTC Beam Search Results
        dev_recognition_results = {}
        dev_best_wer_score = float("inf")
        dev_best_recognition_beam_size = 1

        test_recognition_results = {}
        test_best_wer_score = float("inf")
        test_best_recognition_beam_size = 1

        for rbw in recognition_beam_sizes:
            # DEV
            logger.info("-" * 60)
            dev_valid_start_time = time.time()
            ...
            logger.info("finished in %.4fs ", time.time() - dev_valid_start_time)
            logger.info(
                "[DEV] partition [RECOGNITION] results:\n\t"
                "WER %3.2f\t(DEL: %3.2f,\tINS: %3.2f,\tSUB: %3.2f)",
                dev_recognition_results[rbw]["valid_scores"]["wer"],
                dev_recognition_results[rbw]["valid_scores"]["wer_scores"]["del_rate"],
                dev_recognition_results[rbw]["valid_scores"]["wer_scores"]["ins_rate"],
                dev_recognition_results[rbw]["valid_scores"]["wer_scores"]["sub_rate"],
            )
            ...
            # Test
            logger.info("-" * 60)
            test_valid_start_time = time.time()
            logger.info("[TEST] partition [RECOGNITION] experiment [BW]: %d", rbw)
            test_recognition_results[rbw] = validate_on_data(
                model=model,
                data=test_data,
                batch_size=batch_size,
                use_cuda=use_cuda,
            ...
            logger.info("finished in %.4fs ", time.time() - test_valid_start_time)
            logger.info(
                "[TEST] partition [RECOGNITION] results:\n\t"
                "WER %3.2f\t(DEL: %3.2f,\tINS: %3.2f,\tSUB: %3.2f)",
                test_recognition_results[rbw]["valid_scores"]["wer"],
                test_recognition_results[rbw]["valid_scores"]["wer_scores"]["del_rate"],
                test_recognition_results[rbw]["valid_scores"]["wer_scores"]["ins_rate"],
                test_recognition_results[rbw]["valid_scores"]["wer_scores"]["sub_rate"],
            )
            if test_recognition_results[rbw]["valid_scores"]["wer"] < test_best_wer_score:
                test_best_wer_score = test_recognition_results[rbw]["valid_scores"]["wer"]
                test_best_recognition_beam_size = rbw
                test_best_recognition_result = test_recognition_results[rbw]
                logger.info("*" * 60)
                logger.info(
                    "[TEST] partition [RECOGNITION] results:\n\t"
                    "New Best CTC Decode Beam Size: %d\n\t"
                    "WER %3.2f\t(DEL: %3.2f,\tINS: %3.2f,\tSUB: %3.2f)",
                    test_best_recognition_beam_size,
                    test_best_recognition_result["valid_scores"]["wer"],
                    test_best_recognition_result["valid_scores"]["wer_scores"][
                        "del_rate"
                    ],
                    test_best_recognition_result["valid_scores"]["wer_scores"][
                        "ins_rate"
    ...
    if do_translation:
        ...
        test_best_bleu_score = float("-inf")
        test_best_translation_beam_size = 1
        test_best_translation_alpha = 1

        for tbw in translation_beam_sizes:
            dev_translation_results[tbw] = {}
            test_translation_results[tbw] = {}

            for ta in translation_beam_alphas:
                # DEV
                logger.info("[DEV] partition [Translation] experiment [BS]: %d, [BA]: %d", tbw, ta)
                ...
                logger.info(
                    ...
                    dev_translation_results[tbw][ta]["valid_scores"]["bleu_scores"][
                        "bleu1"
                    ],
                    dev_translation_results[tbw][ta]["valid_scores"]["bleu_scores"][
                        "bleu2"
                    ...
                    dev_translation_results[tbw][ta]["valid_scores"]["chrf"],
                    dev_translation_results[tbw][ta]["valid_scores"]["rouge"],
                )
                ...
                # TEST
                logger.info("[TEST] partition [Translation] experiment [BS]: %d, [BA]: %d", tbw, ta)
                test_translation_results[tbw][ta] = validate_on_data(
                    model=model,
                    data=test_data,
                    batch_size=batch_size,
                    use_cuda=use_cuda,
                ...
                logger.info(
                    ...
                    "CHRF %.2f\t"
                    "ROUGE %.2f",
                    test_translation_results[tbw][ta]["valid_scores"]["bleu"],
                    test_translation_results[tbw][ta]["valid_scores"]["bleu_scores"][
                        "bleu1"
                    ],
                    test_translation_results[tbw][ta]["valid_scores"]["bleu_scores"][
                        "bleu2"
                    ...
                    test_translation_results[tbw][ta]["valid_scores"]["chrf"],
                    test_translation_results[tbw][ta]["valid_scores"]["rouge"],
                )

                if test_translation_results[tbw][ta]["valid_scores"]["bleu"] > test_best_bleu_score:
                    test_best_bleu_score = test_translation_results[tbw][ta]["valid_scores"]["bleu"]
                    test_best_translation_beam_size = tbw
                    test_best_translation_alpha = ta
                    test_best_translation_result = test_translation_results[tbw][ta]
                    logger.info("-" * 60)
                    logger.info(
                        "[TEST] partition [Translation] results:\n\t"
                        "New Best Translation Beam Size: %d and Alpha: %d\n\t"
                        ...
                        "CHRF %.2f\t"
                        "ROUGE %.2f",
                        test_best_translation_beam_size,
                        test_best_translation_alpha,
                        test_best_translation_result["valid_scores"]["bleu"],
                        test_best_translation_result["valid_scores"]["bleu_scores"][
                            "bleu1"
                        ],
                        test_best_translation_result["valid_scores"]["bleu_scores"][
                            "bleu2"
                        ...
                        test_best_translation_result["valid_scores"]["chrf"],
                        test_best_translation_result["valid_scores"]["rouge"],
                    )
                    logger.info("-" * 60)
    ...
    logger.info(
        ...
        "CHRF %.2f\t"
        "ROUGE %.2f",
        test_best_recognition_beam_size if do_recognition else -1,
        test_best_translation_beam_size if do_translation else -1,
        test_best_translation_alpha if do_translation else -1,
        test_best_recognition_result["valid_scores"]["wer"] if do_recognition else -1,
        test_best_recognition_result["valid_scores"]["wer_scores"]["del_rate"]
        if do_recognition
        else -1,
        test_best_recognition_result["valid_scores"]["wer_scores"]["ins_rate"]
        if do_recognition
        else -1,
        test_best_recognition_result["valid_scores"]["wer_scores"]["sub_rate"]
        if do_recognition
        else -1,
        test_best_translation_result["valid_scores"]["bleu"] if do_translation else -1,
        test_best_translation_result["valid_scores"]["bleu_scores"]["bleu1"]
        if do_translation
        else -1,
        test_best_translation_result["valid_scores"]["bleu_scores"]["bleu2"]
        if do_translation
        else -1,
        test_best_translation_result["valid_scores"]["bleu_scores"]["bleu3"]
        if do_translation
        else -1,
        test_best_translation_result["valid_scores"]["bleu_scores"]["bleu4"]
        if do_translation
        else -1,
        test_best_translation_result["valid_scores"]["chrf"] if do_translation else -1,
        test_best_translation_result["valid_scores"]["rouge"] if do_translation else -1,
    )
    logger.info("*" * 60)
    ...
    if output_path is not None:
        if do_recognition:
            ...
            test_gls_output_path_set = "{}.BW_{:03d}.{}.gls".format(
                output_path, test_best_recognition_beam_size, "test"
            )
            _write_to_file(
                test_gls_output_path_set,
                [s for s in test_data.sequence],
                test_best_recognition_result["gls_hyp"],
            )
        ...
        if do_translation:
            if dev_best_translation_beam_size > -1:
                ...
                test_txt_output_path_set = "{}.BW_{:02d}.{}.txt".format(
                    output_path, test_best_translation_beam_size, "test"
                )
            ...
            _write_to_file(
                ...
                [s for s in test_data.sequence],
                test_best_translation_result["txt_hyp"],
            )
        ...
        with open(output_path + ".test_results.pkl", "wb") as out:
            pickle.dump(
                {
                    "recognition_results": test_recognition_results
                    if do_recognition
                    else None,
                    "translation_results": test_translation_results
                    if do_translation
                    else None,
                },
                out,
