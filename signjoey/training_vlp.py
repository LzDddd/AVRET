...
from signjoey.prediction_vlp import validate_on_data, test
...
class TrainManager:
    ...
    def __init__(self, model: SignModel, config: dict) -> None:
        ...
        self.dataset_version = config["data"].get("version", "phoenix_2014t_trans")
        ...
        self.visual_loss_function = KLLoss()
        self.txt_loss_function = KLLoss()
        ...
        self.random_text_masking_ratio = config["data"].get(
            "random_text_masking_ratio", None
        )
        ...
        self.best_bleu_1 = np.inf if self.minimize_metric else -np.inf
        self.best_bleu_2 = np.inf if self.minimize_metric else -np.inf
        self.best_bleu_3 = np.inf if self.minimize_metric else -np.inf
        self.is_equal = (
            lambda score: score[0] == self.best_ckpt_score
            and score[1] >= self.best_bleu_1
            and score[2] >= self.best_bleu_2
            and score[3] >= self.best_bleu_3
        )
    ...
    def init_from_checkpoint(
        ...
        for k in model_checkpoint['model_state'].keys():
            if k in old_model_state.keys():
                old_model_state[k] = model_checkpoint['model_state'][k]

        # restore model and optimizer parameters
        self.model.load_state_dict(old_model_state)   # old_model_state
        ...
        for epoch_no in range(self.epochs):
            ...
            for batch in iter(train_iter):
                ...
                batch = Batch(
                    ...
                    random_text_masking_ratio=self.random_text_masking_ratio
                )
                ...
                if self.steps % self.validation_freq == 0 and update:
                    ...
                    val_res = validate_on_data(
                        ...
                        frame_subsampling_ratio=self.frame_subsampling_ratio,
                        visual_loss_function=self.visual_loss_function,
                        txt_loss_function=self.txt_loss_function
                    )
                    self.model.train()
                    ...
                    if self.is_equal(temp_scores):
                        self.best_ckpt_score = temp_scores[0]
                        self.best_all_ckpt_scores = val_res["valid_scores"]
                        self.best_ckpt_iteration = self.steps
                        self.best_bleu_1 = temp_scores[1]
                        self.best_bleu_2 = temp_scores[2]
                        self.best_bleu_3 = temp_scores[3]
                        self.logger.info(
                            "Hooray! New best validation result [%s]! BLEU-4相同，但其他更好！！！",
                            self.early_stopping_metric,
                        )
    ...
    def _train_batch(self, batch: Batch, update: bool = True) -> (Tensor, Tensor):
        ...
        """
        recognition_loss = -1
        vlp_loss = self.model.get_loss_for_vlp_pretrain_batch(
            batch=batch,
        ...
        if self.do_translation:
            ...
            normalized_translation_loss = vlp_loss / (
...
def train(cfg_file: str) -> None:
    ...
    trainer.logger.info('This is the VLP pretrain version!!!!!!!!!!')
    ...
    ckpt = "{}/{}.ckpt".format(trainer.model_dir, trainer.best_ckpt_iteration)  # trainer.best_ckpt_iteration
