...
def clip_partition(
    ...
    unbind_sign = input_tensor.unbind(0)  # 32 * [475, 512]
    clips = []
...
def mask_partition(
    ...
    :return:
    """
    unbind_mask = mask.unbind(0)  # 32 * [1, 475]
    mask = []
...
def unpadding_clip_partition(
    ...
    for i, clip in enumerate(clips):
        if clip.shape[0] == 1:
            ...
            for clip_frame_no in range(clip.shape[0]):
                if clip_frame_no == 0:
                    front_temp = []
                    for ex_no in range(extension_frames_size):
                        if ex_no + 1 >= clip.shape[0]:
                            break
                        else:
                            temp_ex_t = clip[clip_frame_no+1+ex_no, -1, :]
                            front_temp.append(temp_ex_t)
                    for tpp in front_temp:
                        clips[i][clip_frame_no, -1, :] = clips[i][clip_frame_no, -1, :] + tpp
                elif clip_frame_no == clip.shape[0] - 1:
                    back_temp = []
                    for ex_no in range(extension_frames_size):
                        if clip_frame_no - 1 - ex_no < 0:
                            break
                        else:
                            temp_ex_t = clip[clip_frame_no - 1 - ex_no, 0, :]
                            back_temp.append(temp_ex_t)
                    for btp in back_temp:
                        clips[i][clip_frame_no, 0, :] = clips[i][clip_frame_no, 0, :] + btp
                else:
                    front_temp = []
                    for ex_no in range(extension_frames_size // 2):
                        if clip_frame_no + 1 + ex_no >= clip.shape[0]:
                            break
                        else:
                            temp_ex_t = clip[clip_frame_no + 1 + ex_no, -1, :]
                            front_temp.append(temp_ex_t)
                    for ftp in front_temp:
                        clips[i][clip_frame_no, -1, :] = clips[i][clip_frame_no, -1, :] + ftp
