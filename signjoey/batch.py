...
class Batch:
    ...
    def __init__(
        ...
        random_text_masking_ratio: bool = None,
    ):
        """
        ...
        if random_frame_masking_ratio and is_train:   # VMP pretrain
            # print('start pretrain!!!!!!!!!!!!!!!')
            tmp_sgn = torch.zeros_like(self.sgn).to(self.sgn.device)
            tmp_sgn_lengths = torch.zeros_like(self.sgn_lengths).to(self.sgn_lengths.device)
            ...
            for idx, (features, length) in enumerate(zip(self.sgn, self.sgn_lengths)):
                ...
                if length >= 15:
                    ...
                    max_mask_size = 1
                frame_mask_random_index = random.sample(temp_list, max_mask_size)

                temp_feat = []
                for f_idx in range(length):
                    if f_idx not in frame_mask_random_index:
                        temp_feat.append(features[f_idx, :])
                temp_feat = torch.stack(temp_feat, dim=0).to(self.sgn.device)
                tmp_sgn[idx, :temp_feat.shape[0], :] = temp_feat
                tmp_sgn_lengths[idx] = temp_feat.shape[0]
            self.sgn = tmp_sgn
            self.sgn_lengths = tmp_sgn_lengths
            del tmp_sgn, tmp_sgn_lengths
            gc.collect()

        if is_train:
            if self.sgn.shape[1] < 470:  # 475  pretrain: 470
                temp_sgn = torch.zeros((self.sgn.shape[0], 470, sgn_dim)).to(self.sgn.device)
        ...
        if hasattr(torch_batch, "txt"):
            ...
            # generate random masked txt for VLP pretrain
            self.random_text_masking_ratio = random_text_masking_ratio
            if random_text_masking_ratio:
                if txt_pad_index == 1:
                    random_masked_txt_input = torch.ones_like(self.txt_input).to(self.txt_input.device)
                elif txt_pad_index == 0:
                    random_masked_txt_input = torch.zeros_like(self.txt_input).to(self.txt_input.device)
                else:
                    raise ValueError("Unknown txt_pad_index")

                random_masked_txt_lengths = torch.zeros_like(self.txt_lengths).to(self.txt_lengths.device)

                for idx, (temp_txt, temp_length) in enumerate(zip(self.txt_input, self.txt_lengths)):
                    temp_txt = temp_txt.clone()
                    temp_txt_list = [i for i in range(1, temp_length-1)]

                    if temp_length >= 10:
                        max_txt_mask_size = 3
                    elif 5 <= temp_length < 10:
                        max_txt_mask_size = 2
                    elif 1 < temp_length < 5:
                        max_txt_mask_size = 1
                    else:
                        max_txt_mask_size = 0
                    frame_mask_random_index = random.sample(temp_txt_list, max_txt_mask_size)

                    temp_feat = []
                    for t_idx in range(temp_txt.shape[0]):
                        if t_idx not in frame_mask_random_index:
                            temp_feat.append(temp_txt[t_idx])
                    temp_feat = torch.stack(temp_feat, dim=0).to(self.txt_input.device)
                    random_masked_txt_input[idx, :temp_feat.shape[0]] = temp_feat
                    random_masked_txt_lengths[idx] = temp_length - max_txt_mask_size - 1

                self.random_masked_txt_input = random_masked_txt_input
                self.random_masked_txt_lengths = random_masked_txt_lengths
                self.random_masked_txt_input_mask = (self.random_masked_txt_input != txt_pad_index).unsqueeze(1)
    ...
    def _make_cuda(self):
        ...
        if self.txt_input is not None:
            ...
            if self.random_text_masking_ratio:
                self.random_masked_txt_input = self.random_masked_txt_input.cuda()
                self.random_masked_txt_input_mask = self.random_masked_txt_input_mask.cuda()
    ...
    def sort_by_sgn_lengths(self):
        ...
        if self.txt is not None:
            ...
            if self.random_text_masking_ratio:
                self.random_masked_txt_input = self.random_masked_txt_input[perm_index]
                self.random_masked_txt_input_mask = self.random_masked_txt_input_mask[perm_index]
                self.random_masked_txt_lengths = self.random_masked_txt_lengths[perm_index]
